<script>
	...
	let banner = null; // { type: 'success' | 'pending' | 'error' }
	...
	async function handleSubscribe(plan) {
		...
		try {
			await startCheckout(plan.id, {
				onSuccess: () => (banner = { type: 'success' }),
				onPending: () => (banner = { type: 'pending' }),
				onError: () => (banner = { type: 'error' }),
				onClose: () => (loadingPlanId = null)
			});
		} catch (e) {
			banner = { type: 'error', message: e.message };
		} finally {
			loadingPlanId = null;
		}
	}
...
<Topbar title={$t.pricing.title} breadcrumb={[$t.common.dashboard, $t.pricing.breadcrumb]} />

<main class="mx-auto max-w-5xl px-6 py-8">
	<BackButton fallback="/" />

	<div class="mb-8 text-center">
		<h2 class="text-theme-xl font-semibold text-gray-800 dark:text-white/90">{$t.pricing.heading}</h2>
		<p class="mt-1 text-theme-sm text-gray-400">{$t.pricing.subheading}</p>
	</div>
	...
	{#if banner}
		...
		>
			{banner.message || $t.pricing.banners[banner.type]}
		</div>
	{/if}

	{#if plans.length === 0}
		<p class="py-16 text-center text-theme-sm text-gray-400">{$t.pricing.loadingPlans}</p>
	{:else}
	...
	<p class="mt-8 text-center text-theme-xs text-gray-400">
		...
		<a href="/billing" class="text-brand-500 hover:underline">{$t.pricing.billingLink}</a>.
	</p>
</main>
